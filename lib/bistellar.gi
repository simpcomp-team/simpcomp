...
function(complex1, complex2) 
	...
	if pm1=false then
		Info(InfoSimpcomp,2,"SCEquivalent: complexes should be closed pseudomanifolds");
		return fail;
	fi;
...
function(complex,k) 
	...
	if(k>Int((SCDim(complex)+2)/2)) then
		Info(InfoSimpcomp,1,"SCIsKStackedSphere: second argument must be a positive integer k with 1 <= k <= \\lfloor (SCDim(complex)+2)/2 \\rfloor.");
		return fail;
	fi;
	...
	for i in [1..Size(l)] do
		...
		if IsList(l[i]) and l[i][1] = true then
			if IsBound(l[i-1]) and l[i-1] <= k then
				Info(InfoSimpcomp,1,"SCIsKStackedSphere: complex is even (at least) a ",l[i-1],"-stacked sphere.");
							return l[i];
			fi;
			break;
		fi;
	...
	if(h<>true) then
		Info(InfoSimpcomp,1,"SCIsKStackedSphere: first argument must be a PL manifold -- passed complex is not even a homology sphere.");
	...
	for try in [1..maxtries] do
		...
		if movable then
			...
			Info(InfoSimpcomp,1,"SCIsKStackedSphere: complex is not a closed pseudomanifold.");
					return [false,SCEmpty()];
		fi;
			
		if result=fail then	
			Info(InfoSimpcomp,1,"SCIsKStackedSphere: SCReduceComplexEx returned fail.");
			return fail;
		fi;
		...
		if result[1]=true then
			...
			if f[1]=d+2 and f[1]=f[Size(f)] then
				...
				if(ball=fail) then
					Info(InfoSimpcomp,1,"SCIsKStackedSphere: something is wrong with the facet list of the constructed ball that should be a filled version of the sphere passed as complex. Please contact the authors/maintainers of simpcomp.");
					return fail;
				fi;
				
				SCRename(ball,Concatenation("Filled ",String(k),"-stacked sphere (",String(SCName(complex)),")"));
