...
## discrete Morse theory approach. See <Cite Key="Benedetti13RandomDMT" />, <Cite Key="Adiprasito14RDMTII" /> for details.
## <Example>
...
## <Returns>two lists of small integer lists upon success, <C>fail</C> otherweise.</Returns>
## <Description>
## Builds a discrete Morse function following Adiprasito, Benedetti and Lutz' reverse lexicographic random
## discrete Morse theory approach. See <Cite Key="Benedetti13RandomDMT" />, <Cite Key="Adiprasito14RDMTII" /> for details.
## <Example>
...
## <Description>
## Employs a greedy collapsing algorithm to collapse the simplicial complex <Arg>complex</Arg>. 
...
## <Arg>complex</Arg>. See also <Ref Meth="SCCollapseGreedy" /> and <Ref Meth="SCCollapseRevLex" />.
## <Example>
...
## <Returns>simplicial complex of type <C>SCSimplicialComplex</C> upon success, <K>fail</K> otherwise.</Returns>
## <Description>
## Employs a greedy collapsing algorithm in reverse lexicographical order to collapse the simplicial complex <Arg>complex</Arg>. 
...
## <Returns>a random Morse function of a simplicial complex and a list of critical faces.</Returns>
## <Description>
...
function(cc)
	...
	if ForAny(HD[1][Size(HD[1])],x->Size(x) < 2) then
		Info(InfoSimpcomp,1,"SCMorseUST: first argument has boundary or is not a weak pseudomanifold, try other heuristic.");
		return fail;
	fi;
	...
	while Size(visited) < Size(c[Size(c)]) do
		...
		while vColl <> [] do
			...
			Remove(available[Size(available)],PositionSorted(available[Size(available)],vColl[l[mx]]));
		...
		od;
	        while available[iii+1] <> [] do
	            if free=[] then
	                ...
	                Remove(available[iii+1],PositionSorted(available[iii+1],pairedFace));
	                R:=downward[iii][pairedFace];
	                # update upward Hasse diagram
	                for i in R do
	                    Remove(upward[iii][i],Position(upward[iii][i],pairedFace));
...
## <Description>
## Computes <Arg>iter</Arg> versions of a discrete Morse function of <Arg>complex</Arg> using a randomised 
## method specified by <Arg>morsefunc</Arg> (default choice is <Ref Func="SCMorseRandom"/>,
## other randomised methods available are <Ref Func="SCMorseRandomLex"/>
## <Ref Func="SCMorseRandomRevLex"/>, and <Ref Func="SCMorseUST"/>). The
## result is referred to by the Morse spectrum of <Arg>complex</Arg> and is returned
## in form of a list containing all Morse vectors sorted by number of critical points 
## together with the actual vector of critical points and how often they ocurred 
## (see <Cite Key="Benedetti13RandomDMT" /> for details).
## <Example>
...
function(arg)
    ...
    local i,f,Spec,s,Vec,pos,
	c,morsefunc,iter,tmp;

    if Size(arg) > 3 or Size(arg) < 2 then
	Info(InfoSimpcomp,1,"SCMorseSpec: no. of arguments must be 2 or 3.");
	return fail;
    fi;
	...
	Info(InfoSimpcomp,1,"SCMorseSpec: first argument must be of type SCSimplicialComplex, second argument must be a positive integer.");
	return fail;
    fi;
...
SCIntFunc.SCBdry:=function(chain)
	...
	for elm in chain do
		...
		for i in tmp[1] do
			if i in bdry then
				coeffs[Position(bdry,i)]:=coeffs[Position(bdry,i)]+tmp[2][Position(tmp[1],i)]*elm[2];
			else
				Add(bdry,i);
...
SCIntFunc.SCAddCrit:=function(sum,chain)
	...
	od;
	Info(InfoSimpcomp,1,"SCIntFunc.SCAddCrit: second argument must correspond to an item in first argument.");
	return fail;
end;
...
SCIntFunc.SCFindGradientPaths:=function(f,orFace,crits,dict)
	...
	# discrete gradient, takes an (i-)face and computes the unique matched (i+1)-face
	SCGradient:=function(f,face)
		...
		matching:=[];
		for i in [1..pos-1] do
			if not IsSubset(f[i],face) or not Size(f[i]) = Size(face)+1 then continue; fi;
			matching:=f[i];
			break;
		od;
		if matching = [] then
	...
	# if current face has a matched face
		...
		# remove original face (-> outgoing discrete gradient flow \Phi, coming from 'orFace')
		# outChain = \Phi (orFace) 
...
SCIntFunc.SCMorseBdryOperator:=function(f,dimension)
	local i,ii,iii,tau,faces,new,sum,sum1,critsUp,critsDown,innerprod,dict,row,tau2;
...
## <Returns>a list of pairs of the form <C>[ integer, list ]</C> upon success, fail otherwise.</Returns>
## <Description>
## Computes the homology groups of a given simplicial complex <Arg>c</Arg> using the function
## <Arg>morsechoice</Arg> for discrete Morse function computations and <Arg>smithchoice</Arg>
## for Smith normal form computations.<P/>
##
...
## <Returns>a list of pairs of the form <C>[ integer, list ]</C> upon success</Returns>
## <Description>
...
function(HD,top)
	...
	if not IsBool(top) then
		Info(InfoSimpcomp,1,"SCSpanningTreeRandom: second argument must be of type IsBool.");
		return fail;
	fi;

	if not Size(HD)=2 or not ForAll(HD,x->IsList(x)) then
		Info(InfoSimpcomp,1,"SCSpanningTreeRandom: first argument must be a pair of lists.");
		return fail;
	fi;

	if ForAny(HD[1][Size(HD[1])],x->Size(x) <> 2) and top then
		Info(InfoSimpcomp,1,"SCSpanningTreeRandom: first argument has boundary or is not a weak pseudomanifold, try top = false.");
		return fail;
	fi;

	if Size(HD[1]) < 2 or Size(HD[2]) < 2 then
		Info(InfoSimpcomp,1,"SCSpanningTreeRandom: first argument must be a Hasse diagram of a d-dimensional simplicial complex, d > 1.");
		return fail;
	fi;
...
SCIntFunc.FundGroupFindGradientPaths:=function(f,orFace,crits,dict)
	...
	for i in [1..Size(sum)] do
		...
		if (i < Size(sum)) and (sum[i][1] = sum[i+1][1] and sum[i][2] = (-1)*sum[i+1][2]) then
			skip:=true;
			continue;
		fi;
		Add(sum1,sum[i]);
	...
	AddDictionary(dictCopy,[orFace[1],(-1)*orFace[2]],List(Reversed(sum1),x->[x[1],(-1)*x[2]]));
	...
	for elm in up do
		...
		for ii in faces do
			if ii[1] in down then
				...
				gradPaths:=SCIntFunc.FundGroupFindGradientPaths([Morse,critical],ii,down,dict);
				skip:=false;
				for i in [1..Size(gradPaths[1])] do
					...
					if (i < Size(gradPaths[1])) and (gradPaths[1][i][1] = gradPaths[1][i+1][1] and gradPaths[1][i][2] = (-1)*gradPaths[1][i+1][2]) then
						skip := true;
						continue;
					fi;
...
function(arg)
	...
	if Size(arg) < 1 or Size(arg) > 4 then
		Info(InfoSimpcomp,1,"SCIsSimplyConnectedEx: no. or arguments must be 1, 2, 3, or 4.");
		return fail;
	fi;
	...
	if not SCIsSimplicialComplex(c) or not IsBool(top) or not IsPosInt(tries) then
		Info(InfoSimpcomp,1,"SCIsSimplyConnectedEx: first argument must be of type SCIsSimplicialComplex.");
		return fail;
	fi;
	...
	if dim < 1 then
		...
		if Filtered(f[2],x->Size(x)=2) = [] then
			...
			Info(InfoSimpcomp,1,"SCIsSimplyConnectedEx: Could not determine collapsing seuqence, build fundamental group from Morse function instead.");
			if [] in f[1] then
...
#			Print(tmp,"ms for fundamental group (conventional)\n");		
			...
			if GeneratorsOfGroup(g) = [] then
				...
				Info(InfoSimpcomp,1,"SCIsSimplyConnectedEx: could not determine or disprove simple connectivity. Returning fundamental group. You might wanna try IsTrivial(g) to determine whether complex is simply connected.");
				return [fail,g];		
			fi;
		fi;
	fi;	
...
function(arg)
	...
	if Size(arg) < 1 or Size(arg) > 3 then
		Info(InfoSimpcomp,1,"SCIsManifoldEx: no. or arguments must be 1, 2, or 3.");
		return fail;
	fi;
	...
	if not SCIsSimplicialComplex(c) or not IsBool(quasi) or not IsBool(aut) then
		Info(InfoSimpcomp,1,"SCIsManifoldEx: first argument must be of type SCIsSimplicialComplex, second and third argument must be of type IsBool.");
		return fail;
	fi;
	...
	for i in Reversed([1..dim-2]) do
		...
		if quasi = false and i = dim-4 then
			for j in [1..Size(orbits[i])] do;
				...
				if sc = false then
					Info(InfoSimpcomp,1,"SCIsManifoldEx: found non-simply connected link ",orbits[i][j],".");
					return false;
				fi;
				lk:=SCReduceComplex(lk);
				if lk[1] <> true or (lk[1] = true and SCFVector(lk[2]) <> [6,15,20,15,6]) then
					Info(InfoSimpcomp,1,"SCIsManifoldEx: could not reduce link ",orbits[i][j],".");
					return fail;
				fi;
			od;
		fi;
		...
		for j in [1..Size(orbits[i])] do
			...
			if i = dim-2 then
				...
				if sc <> true then
					Info(InfoSimpcomp,1,"SCIsManifoldEx: could not determine simply connectivity of link ",orbits[i][j],".");
					return fail;
				fi;

				hom:=SCHomology(lk);
				if ForAny(hom{[1..Size(hom)-1]},x->x <> [0,[]]) or hom[Size(hom)] <> [1,[]] then
					return false;
				fi;
			fi;
		od;
	...
	if quasi then
		Info(InfoSimpcomp,1,"SCIsManifoldEx: WARNING: assuming Smooth Poincare conjecture.");
	fi;
...
function(c)
	...
	if dim = 4 then
		Info(InfoSimpcomp,1,"SCIsSphere: WARNING, result is only valid up to homeomorphy.");
	fi;
