...
function(complex, modulus)
	...
	for k in [2..d+1] do
		matrices[k-1] :=         HomalgInitialMatrix(Length(skel[k]),Length(skel[k-1]),ring);
		...
		for i in [1..Length(skel[k])] do
			...
			for j in [1..Length(b)] do
				...
				if(pos<>fail) then
					...
					Info(InfoSimpcomp,1,"SCHomalgBoundaryOperatorMatrices: list of boundary skel[k-1] is not complete k=",k,", b[j]=",b[j],"!");
					return fail;
				fi;
			od;
		od;
		...
		for i in [1..Length(skel[k])] do
			...
			for j in [1..Length(b)] do
				...
				if(pos<>fail) then
					...
					Info(InfoSimpcomp,1,"SCHomalgCoboundaryOperatorMatrices: list of boundary skel[k] is not complete!");
					return fail;
				fi;
			od;
		od;
...
function(complex,modulus)
	...
	Info(InfoSimpcomp,2,"SCHomalgHomologyOp: computing ",rnam,"-homology ranks...");	
...
function(complex,modulus)
	...
	Info(InfoSimpcomp,1,"SCHomalgHomologyBasisOp: constructed ",rnam,"-homology groups.");
	return Homology(C);
end);
...
function(complex,modulus)
	...
	Info(InfoSimpcomp,2,"SCHomalgCohomologyOp: computing ",rnam,"-cohomology ranks...");
...
function(complex,modulus)
	...
	Info(InfoSimpcomp,1,"SCHomalgCohomologyBasisOp: constructed ",rnam,"-cohomology groups.");
	return Cohomology(C);
end);
